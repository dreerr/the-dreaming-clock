 ...
 *
 *  The BMA280 is an inexpensive (~$1), three-axis, high-resolution (14-bit)
 * acclerometer in a tiny 2 mm x 2 mm LGA12 package with 32-slot FIFO, two
 * multifunction interrupts and widely configurable sample rate (15 - 2000 Hz),
 * full range (2 - 16 g), low power modes, and interrupt detection behaviors.
 * This accelerometer is nice choice for low-frequency sound and vibration
 * analysis, tap detection and simple orientation estimation.
 *
 ...
 * choices are:
      ...
      BW_7_81Hz, BW_15_63Hz, BW_31_25Hz, BW_62_5Hz, BW_125Hz, BW_250Hz,
 BW_500Hz, BW_1000Hz normal_Mode, deepSuspend_Mode, lowPower_Mode, suspend_Mode
      sleep_0_5ms, sleep_1ms, sleep_2ms, sleep_4ms, sleep_6ms, sleep_10ms,
 sleep_25ms, sleep_50ms, sleep_100ms, sleep_500ms, sleep_1000ms
*/
uint8_t Ascale = AFS_2G, BW = BW_125Hz, power_Mode = normal_Mode,
        sleep_dur = sleep_0_5ms, tapStatus, tapType;
...
uint32_t delt_t = 0, sensorCount = 0;  // used to control display output rate
...
void myinthandler1() {
  newData = true;  // Just set flag when interrupt received, don't try reading
                   // data in an interrupt handler
}

void myinthandler2() { newTap = true; }
...
void setupSensor() {
  ...
  if (c == 0xFB)  // CHIP_ID should always be 0xFB
  {
    ...
    aRes = BMA280.getAres(
        Ascale);  // get sensor resolutions, only need to do this once
    BMA280.selfTestBMA280();  // perform sensor self test
    BMA280.resetBMA280();     // software reset before initialization
    delay(1000);              // give some time to read the screen
    BMA280.initBMA280(Ascale, BW, power_Mode,
                      sleep_dur);     // initialize sensor for data acquisition
    BMA280.fastCompensationBMA280();  // quickly estimate offset bias

    attachInterrupt(intPin1, myinthandler1,
                    RISING);  // define interrupt for INT1 pin output of BMA280
    attachInterrupt(intPin2, myinthandler2,
                    RISING);  // define interrupt for INT2 pin output of BMA280

  }

  else
    Serial.print("BMA280 is not answering...0x");
...
void loopSensor() {
  if (newData == true) {  // On interrupt, read data
    ...
    ax = (float)accelCount[0] * aRes /
         4.0f;  // get actual g value, this depends on scale being set
  ...
  delt_t = millis() - sensorCount;
  if (delt_t >
      500) {  // update serial once per half-second independent of read rate
    ...
    temperature = 0.5f * ((float)tempCount) +
                  23.0f;  // Gyro chip temperature in degrees Centigrade
